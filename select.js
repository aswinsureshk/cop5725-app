...
var doquery_array = function (conn, cb) {
  conn.execute(
    "SELECT REGION_ID,REGION_NAME FROM REGION WHERE REGION_NAME LIKE 'W%' ORDER BY REGION_NAME",
    function(err, result) {
      if (err) {
        return cb(err, conn);
      } else {
        console.log("----- Regions beginning with 'W' (default ARRAY output format) --------");
  ...
  conn.execute(
    ...
    function(err, result) {
      if (err) {
        ...
        console.log("----- Regions beginning with 'W' (Object output format) --------");
        console.log(result.rows);
        return cb(null, conn);
      }
    });
